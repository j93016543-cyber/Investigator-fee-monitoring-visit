...
/* A — 기관별 CTA 변경이력 & Effective date (전체 개요) */
function renderCTAHistory(p){
  const scs=(DATA.site_cta||{}).sites||{};
  const sites=Object.keys(scs).filter(s=>(scs[s].ctas||[]).length).sort((a,b)=>String(a).localeCompare(String(b)));
  if(!sites.length){ return; }
  const totVers=sum(sites,s=>scs[s].ctas.length);

  // 개요: site 1행
  const ovCols=[{k:'site',h:'Site#'},{k:'name',h:'기관'},{k:'n',h:'버전',num:true},
    {k:'first',h:'최초 Effective'},{k:'eff',h:'현재 유효 CTA'},{k:'effd',h:'현재 Effective'}];
  const ovRows=sites.map(s=>{ const cs=scs[s].ctas.slice().sort((a,b)=>String(a.effective_date||'').localeCompare(b.effective_date||''));
    const withDate=cs.filter(c=>c.effective_date);
    const eff=ctaAt(s,null);
    return {site:s, name:siteName(s)||scs[s].name||'', n:cs.length,
      first: withDate.length?withDate[0].effective_date:'—',
      eff: eff?eff.version:'—', effd: eff?(eff.effective_date||'—'):'—'}; });

  // 변경이력 상세: site별 섹션 + 버전 (Δ항목)
  const logCols=[{k:'v',h:'CTA Version'},{k:'e',h:'Effective Date'},{k:'pv',h:'Protocol Ver.'},{k:'n',h:'항목수',num:true},{k:'d',h:'Δ항목',num:true}];
  const logRows=[];
  sites.forEach(s=>{
    logRows.push({v:`Site ${s} — ${siteName(s)||scs[s].name||''}`, e:'',pv:'',n:'',d:'', _cls:'sec'});
    const cs=scs[s].ctas.slice().sort((a,b)=>String(a.effective_date||'').localeCompare(b.effective_date||''));
    const eff=ctaAt(s,null);
    let prev=null;
    cs.forEach(c=>{ const n=(c.items||[]).length; const delta= prev==null?null:(n-prev);
      logRows.push({v:c.version, e:c.effective_date||'—', pv:c.protocol_version||'—', n:n||'—',
        d: delta==null?'—':(delta>0?'+'+delta:String(delta)),
        _cls:(eff&&c.version===eff.version)?'eff':''});
      prev=n; });
  });

  const body=el('div',{},[
    el('div',{class:'kpis'},[
      kpi('CTA 관리 기관', sites.length+'개', '연구비 A 하위그룹'),
      kpi('총 CTA 버전', totVers+'개', '전 기관 누적'),
      kpi('미입력 기관', String([...new Set([...(DATA.investigator_fees||[]),...(DATA.pass_through_invoices||[])].map(x=>String(x.site)).filter(Boolean))].filter(s=>!(scs[s]&&(scs[s].ctas||[]).length)).length), 'CTA 폴더 대기'),
    ]),
    el('h3',{class:'subh'},'기관별 CTA 현황 (한눈에)'),
    tableFrom(ovCols, ovRows),
    el('h3',{class:'subh'},'CTA 변경이력 (Effective date 순 · Δ항목 = 직전 버전 대비 항목 증감)'),
    tableFrom(logCols, logRows, {tall:true}),
  ]);
  p.append(card('A','기관별 CTA 변경이력 · Effective date','전 기관 계약 버전 타임라인', body));
}
...
function renderA(p, fees, invs){
  ...
  p.append(card('A','기관별 CTA 상세 · 지급 요약', 'Site별 · Total Investigator fee · 분기별 · 적용 CTA · 항목별 비용', el('div',{},[ctrl,holder])));
}
